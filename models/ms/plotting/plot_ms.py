...
"""
Plot functions for the Mitchell-Schaeffer example of the PDAF test
...
def interpretParameters(P):
    labs = ["d", "k", "ti", "tu", "to", "tc", "ug"]
    lab = ""
    for (l,p) in zip(labs,P):
        lab += l + "=" + str(p) + ","
    return lab
...
def plot_example():
    ...
    the Mitchell-Schaeffer model provided in tools/runasml.sh.
...
def plot_eofs(filename, index):
    ...
    Mitchell-Schaeffer model and plots the selected eigenvectors  or the mean state.
    ...
    with nc.Dataset(filename) as ncfile:
        print("File contains {0:d} eigenvectors.".format(
                ncfile.dimensions['dim_ens'].size
                                                         )
              )
        if index > 0:
    ...
    if index > 0:
        ax.set_title("Eigenvector {0:d} of Mitchell-Schaeffer model".format(index))
    else:
        ax.set_title("Mean state of Mitchell-Schaeffer model")


def plot_obs(filename, timestep, obsmask = None):
    """
    Open a NetCDF file holding observations for the Mitchell-Schaeffer model and
    ...
    if obsmask == None:
        ...
        mask = np.loadtxt(obsmask, dtype=bool)

    tmp = np.arange(1, len(obs)+1)

    fig, ax = plt.subplots()
    ax.plot(tmp[mask], obs[mask], 'b+-')
    ax.set_title(("Observations for Mitchell-Schaeffer model at time {0:.3f} "
...
def plot_rms(filename, plot_forecast=False, plot_analysis=True):
    ...
    the Mitchell-Schaeffer model and plos the true and estimated rms errors.
...
def plot_sigma(filename):
    ...
    the Mitchell-Schaeffer model and plot the eigenvalue spectrum.
    ...
    ax.set_title("Eigenvalues of covariance matrix for Mitchell-Schaeffer model")


def plot_state(filename, timestep, choice='t'):
    """
    Open NetCDF output from the Mitchell-Schaeffer model and plot the state at
    the selected time.
    ...
    nV = 2
    nP = 7
    nX = int((len(state)-nP)/2)
    u = state[:nX]
    v = state[nX:nV*nX]
    p = state[nV*nX:]
    fig, ax = plt.subplots()
    #ax.plot(range(1, len(state)+1), state, 'r')
    ax.plot(range(0, nX), u, 'r', label='u')
    ax.plot(range(0, nX), v, 'g', label='v')
    ax.set_ylim([-0.1, 1.1])
    ax.set_title(("Mitchell-Schaeffer model {0} at time {1:.3f} (time step {2:d})\n"
                  " {3}").format(statestr, time, step, interpretParameters(p))
                 )


def plot_diff(file1, file2, timestep, choice1='t', choice2='a'):
    """
    Open NetCDF output from the Mitchell-Schaeffer model and plot the difference
